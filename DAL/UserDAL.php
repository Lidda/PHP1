	...
	class UserDAL {
		...
		function GetAllUsersDB(){
			$sql = "SELECT users.userID as id, users.emailAddress as email, users.password as password, users.firstName as firstName, 
				...
				users.townName as townName, users.isActive as isActive, users.isAdmin as isAdmin
				FROM users;";
			...
			if (mysqli_num_rows($result) > 0){
				while($row = mysqli_fetch_assoc($result)) {
					...
					$user = new User($id, $email, $password, $firstName, $lastName, $registrationDate, 
								$townName, $birthDate, $isActive, $isAdmin);
					$users[] = $user;
				}
				return $users;
			} else { 
		...
		function InsertUserDB($email, $password, $firstName, $lastName, $registrationDate, $birthDate, $townName){
			...
			$sql = "INSERT INTO users (emailAddress, password, firstName, lastName, registrationDate, birthDate, townName)
					VALUES ('$email', '$password', '$firstName', '$lastName', '$registrationDate', '$birthDate', '$townName');";
